...
<tr>
        ...
        <td>  @if(isset($modelo->ULTIMO_SERIE))
        ...
        <script type = "text/javascript" language = "javascript">
                $(document).ready(function() {
                    ...
                    $("#driver").click(function(event){
                        $('#test').show();
                        {{--$.getJSON("{{url('/trazabilidad/find/OP-107790?json')}}", function() {--}}
                            {{--$('#stage').html('<p> Op:'+wip_ot['nro_op']+' </p>');--}}
                            {{--$('#stage').append('<p>START_QUANTITY: ' + START_QUANTITY + '</p>');--}}
                            {{--$('#stage').append('<p> QUANTITY_COMPLETED: ' + QUANTITY_COMPLETED+ '</p>');--}}
                        {{--});--}}
                    });
