            ...
            var option_series = [
                {
                    ...
                    data: [
                        @foreach($resume->proyectadoCone->groupBy('fecha') as $fecha => $period)
                            @foreach($period->groupBy('periodo') as $periodo => $data)
                                {
                                    x: moment("{{ $fecha }} {{ $periodo  }} -0000", "YYYY-MM-DD HH:mm:ss Z").valueOf(),
                                    y: {{ $data->sum('proyectado') }}
                                },
                            @endforeach
                        @endforeach
                    ]
