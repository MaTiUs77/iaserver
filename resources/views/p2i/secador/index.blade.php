@extends('angular')
@section('ng','app')
@section('title','P2i - Lista de registros de secador')
@section('body')
    @include('p2i.common.header')
    @include('p2i.common.bread',['bread'=>['Secador','Ver registros']])
    ...
    @if(count($proceso)>0)
        <h3>En proceso</h3>
        @include('p2i.secador.partial.table',['secador'=>$proceso])
    @endif

    @if(count($finalizado)>0)
        <h3>Finalizado</h3>
        @include('p2i.secador.partial.table',['secador'=>$finalizado])
    @endif
